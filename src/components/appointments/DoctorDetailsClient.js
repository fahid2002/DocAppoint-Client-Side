      ...
      <div className="det-hero">
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col sm:flex-row gap-8 items-end">
          <div className="det-photo shrink-0">
            <Image src={doc.img} alt={doc.name} width={200} height={240} className="w-full h-full object-cover object-top" />
          </div>
          <div className="flex-1 pb-2">
            <p className="font-['Sora'] text-[11px] font-bold text-[var(--acc3)] uppercase tracking-widest mb-2">{doc.specialty}</p>
            <h2 className="font-['Sora'] text-[clamp(22px,3vw,32px)] font-black text-white mb-2 tracking-tight">{doc.name}</h2>
            <p className="text-[#BA7517] text-sm mb-2.5">
              {stars(doc.rating)}{" "}
              <span className="text-white/60 text-xs">{doc.rating} ({doc.reviews} reviews)</span>
            </p>
            <div className="flex flex-wrap gap-2 mb-4">
              {[
                ...
                <span key={t.label} className="inline-flex items-center gap-1.5 bg-white/10 border border-white/20 rounded-md px-2.5 py-1 text-xs text-white/85 font-semibold">
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          {[
            ...
            <div key={b.lbl} className="bg-[var(--card)] border border-[var(--card-bdr)] rounded-[var(--r-lg)] p-5">
              <p className="text-[10.5px] font-bold text-[var(--tx3)] uppercase tracking-wide mb-1.5">{b.lbl}</p>
          ...
          <div className="sm:col-span-2 bg-[var(--card)] border border-[var(--card-bdr)] rounded-[var(--r-lg)] p-5">
            <p className="text-[10.5px] font-bold text-[var(--tx3)] uppercase tracking-wide mb-2">About the doctor</p>
            <p className="text-sm text-[var(--tx2)] leading-relaxed">{doc.description}</p>
          </div>

          <div className="sm:col-span-2 bg-[var(--card)] border border-[var(--card-bdr)] rounded-[var(--r-lg)] p-5">
            <p className="text-[10.5px] font-bold text-[var(--tx3)] uppercase tracking-wide mb-3">Available time slots</p>
            <div className="flex flex-wrap gap-2 mb-3">
              {doc.availability.map((a) => (
                <span key={a} className="bg-[var(--p-badge-bg)] text-[var(--p-badge-tx)] text-xs font-semibold px-3 py-1 rounded-md">
            ...
            <div className="flex flex-wrap gap-1.5">
              {doc.times.map((t) => (
                <span key={t} className="bg-[var(--green-badge-bg)] text-[var(--green-badge-tx)] text-[11px] font-semibold px-2.5 py-0.5 rounded">{t}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          {session?.user && hasBooked && !alreadyReviewed && (
            <div className="bg-[var(--card)] border border-[var(--card-bdr)] rounded-[var(--r-lg)] p-5 mb-4">
              ...
              <div className="flex gap-1.5 mb-3">
                {[1, 2, 3, 4, 5].map((s) => (
                  <button key={s} onClick={() => setReviewForm((p) => ({ ...p, rating: s }))} className="text-[22px] bg-transparent border-none cursor-pointer transition-colors duration-150" style={{ color: s <= reviewForm.rating ? "#BA7517" : "var(--bdr)" }}>★</button>
                ))}
              </div>
              <textarea
          ...
          {reviews.length === 0 ? (
            ...
            reviews.map((r) => (
              <div key={r._id} className="bg-[var(--card)] border border-[var(--card-bdr)] rounded-[var(--r-lg)] p-5 mb-3 flex gap-3.5">
                ...
                <div className="flex-1">
                  <p className="font-['Sora'] text-[13px] font-bold text-[var(--tx)] mb-0.5">{r.userName}</p>
        ...
        <div className="modal-bg" onClick={(e) => e.target === e.currentTarget && setModalOpen(false)}>
          <div className="modal">
            <div className="flex justify-between items-center mb-5">
              ...
              <button onClick={() => setModalOpen(false)} className="w-8 h-8 rounded-[var(--r-md)] border-[1.5px] border-[var(--bdr)] bg-transparent cursor-pointer flex items-center justify-center text-[var(--tx3)]">
                <i className="ti ti-x" />
              </button>
            </div>
            ...
            <div className="flex items-center gap-2.5 bg-[var(--bg3)] rounded-[var(--r-md)] p-3.5 mb-5">
              ...
              <div>
                ...
                <p className="text-xs text-[var(--p)] font-semibold">{doc.specialty} · ৳{doc.fee}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <div className="auth-field">
                <label>Gender</label>
                <select value={form.gender} onChange={(e) => setForm((p) => ({ ...p, gender: e.target.value }))} className="text-[13.5px] px-3 py-[11px] rounded-[var(--r-md)] border-[1.5px] border-[var(--bdr)] bg-[var(--bg3)] text-[var(--tx)] outline-none w-full">
                  <option>Male</option>
              ...
              <div className="auth-field">
                ...
                <input value={form.phone} onChange={(e) => setForm((p) => ({ ...p, phone: e.target.value }))} placeholder="017XXXXXXXX" />
              </div>

              <div className="auth-field">
                <label>Appointment date *</label>
                <input type="date" value={form.appointmentDate} onChange={(e) => setForm((p) => ({ ...p, appointmentDate: e.target.value }))} min={new Date().toISOString().split("T")[0]} />
              </div>

              <div className="auth-field sm:col-span-2">
                <label>Preferred time slot</label>
                <select value={form.appointmentTime} onChange={(e) => setForm((p) => ({ ...p, appointmentTime: e.target.value }))} className="text-[13.5px] px-3 py-[11px] rounded-[var(--r-md)] border-[1.5px] border-[var(--bdr)] bg-[var(--bg3)] text-[var(--tx)] outline-none w-full">
                  {doc.times.map((t) => <option key={t}>{t}</option>)}
                </select>
              </div>
            </div>

            <button onClick={handleBook} disabled={saving} className="btn btn-primary w-full py-3 text-sm font-bold mt-3">
