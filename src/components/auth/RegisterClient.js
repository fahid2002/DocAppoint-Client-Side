  ...
  return (
    <div className="auth-wrap">
      <div className="auth-card">
        ...
        <h2 style={{ fontFamily: "Sora, sans-serif", fontSize: 22, fontWeight: 900, color: "var(--tx)", textAlign: "center", marginBottom: "0.25rem" }}>Create account</h2>
        <p style={{ fontSize: 13, color: "var(--tx3)", textAlign: "center", marginBottom: "1.4rem" }}>Join thousands of patients on DocAppoint</p>

        <button onClick={handleGoogle} disabled={loading} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 9, width: "100%", padding: 11, border: "1.5px solid var(--bdr)", borderRadius: "var(--r-md)", fontFamily: "Sora, sans-serif", fontSize: 13, fontWeight: 600, cursor: "pointer", background: "var(--card)", color: "var(--tx)", marginBottom: "0.5rem" }}>
        ...
        <div style={{ fontSize: 13, color: "var(--tx2)", textAlign: "center", marginTop: "1rem" }}>
          Already have an account? <Link href="/login" style={{ color: "var(--p)", fontWeight: 700 }}>Login</Link>
        </div>
      </div>
    </div>
  );
